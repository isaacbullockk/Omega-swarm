...
const server = http.createServer((req, res) => {
  ...
  // API: Social accounts (from Buffer if connected, else mock)
  if (pathname === "/api/trpc/social.list") {
    ...
    try {
      ...
      const channels = (chanData.data?.channels || []).map((ch: any) => ({
        id: ch.id,
        platform: ch.service === "instagram" ? "instagram" : ch.service === "youtube" ? "youtube" : "facebook",
        accountName: ch.name,
        handle: "@" + ch.name.toLowerCase().replace(/\s/g, ""),
        connected: true,
      }));
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ result: { data: channels } }));
    } catch (e) {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ result: { data: [] }}));
    }
  ...
  if (pathname === "/api/buffer/channels") {
    ...
    if (!bufferKey) {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ connected: false, message: "Add BUFFER_API_KEY env var", channels: [] }));
      return;
    }
    try {
      const accountRes = await fetch("https://api.buffer.com/graphql", {
  ...
  // API: Post via Buffer (with copy-paste fallback)
  if (pathname === "/api/buffer/post" && req.method === "POST") {
    ...
    req.on("end", async () => {
      try {
        ...
        if (!bufferKey) {
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ success: false, fallback: true, text, message: "Buffer not configured" }));
          return;
        }
        ...
        if (typename === "UnexpectedError" || data.errors) {
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ success: false, fallback: true, text, message: "Posted to Buffer draft. Use copy-paste to publish now." }));
        } else {
          res.writeHead(200, { "Content-Type": "application/json" });
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ success: false, fallback: true, text: body, message: e.message }));
      }
    });
    return;
  }
